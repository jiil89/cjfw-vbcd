...
이 문서는 `prompts/4-prd.md`(PRD)와 `prompts/1-domain-definition-meeting-room-agent.md`(도메인 정의서), `supabase/migrations/`(실제 DB 스키마)에서 이미 확정된 결정사항을 바탕으로, 실제 코드를 작성할 때 따라야 할 구조 원칙을 정리한 것이다. 여기 나오는 원칙은 전부 이 프로젝트의 실제 스택(React 19 + Zustand + TanStack Query, Node.js + Express + `pg`, Vercel, Supabase, Playwright)에 한정된다 — 범용 아키텍처론을 늘어놓지 않는다.

이 문서는 실제 구현이 진행되면서 계속 다듬어진다 — 실사용에서 발견된 버그의 재발 방지 규칙도 여기에 누적한다(진행 상태는 `prompts/4-prd.md` 3번, 세부 이력은 `prompts/9-plan.md` 참고).
...
- **두 종류의 "비밀번호"를 코드 레벨에서도 절대 섞지 않는다.** DB가 이미 `users.encrypted_password`(**CJ WORLD PW**, 복호화 가능한 암호화)와 `users.app_password_hash`(이 서비스 로그인, 단방향 해시)를 분리해 놓았다. 애플리케이션 코드에서도:
  ...
  - **변경 경로도 따로 둔다** — 비밀번호 변경 API/폼을 하나로 합치지 않는다(`PATCH /me/cj-world-password` vs `PATCH /me/app-password`). 검증 방식 자체가 다르기 때문이다: CJ WORLD PW는 **외부 시스템에 실제 로그인해서** 맞는지 확인하고, 앱 비밀번호는 **저장된 해시와 비교**한다.
  - 사용자에게 보이는 명칭은 **CJ WORLD ID / CJ WORLD PW**로 통일한다(DB 컬럼명은 그대로).
- **[20260816 추가] 외부 시스템 자격증명은 저장 전에 그 시스템으로 검증한다.** CJ WORLD PW를 검증 없이 저장하면 오타 하나로 이후 모든 조회·예약이 실패하는데, 사용자는 원인을 알 수도 되돌릴 수도 없다. 검증에 성공한 값만 저장하고, 옛 자격증명으로 맺어둔 세션 캐시는 즉시 버린다.
- **[20260816 추가] 복구 경로는 그 기능이 필요한 상태에서 도달 가능해야 한다.** 로그인은 CJ 세션 확보 실패 시 거부되므로, CJ WORLD PW 재등록을 **인증된 화면 안에만** 두면 정작 필요한 순간(비밀번호가 만료돼 로그인이 막힌 상태)에 쓸 수 없다. 그래서 로그인 전 경로(`POST /auth/cj-world-password`, 앱 로그인 비밀번호로 본인 확인)를 함께 둔다. **기능을 만들 때 "이게 필요한 상황에서 실제로 접근 가능한가"를 반드시 확인할 것.**
...
- **CJ WORLD 자격증명 복호화는 CJ 자동화 계층 안에서만 일어난다.** 다른 계층(도구 계층, API 라우트, 프론트엔드)은 복호화된 CJ 비밀번호를 절대 넘겨받지 않는다.
...
- **[2026-08-14 실사용 버그로 추가] `timestamptz` 컬럼은 리포지토리 계층에서 반드시 실제 문자열(ISO)로 정규화한다.** node-postgres는 `timestamptz`를 JS `Date` 객체로 돌려주는데, `Reservation.startAt: string`처럼 타입을 `string`으로 선언만 하고 실제 변환을 안 하면 그 타입은 거짓말이 된다 — HTTP 응답은 `JSON.stringify`가 Date를 자동으로 문자열화해줘서 겉으론 안 드러나지만, 백엔드 안에서 그 값에 `.slice()`/`.split()` 같은 문자열 메서드를 직접 호출하는 코드는 조용히 죽는다(실제로 예약 변경/취소 대상 특정이 이 버그로 항상 실패했었다 — `prompts/9-plan.md` FE-5 섹션 참고). 리포지토리의 row-매핑 함수(`toReservation()` 등)에서 항상 `.toISOString()`으로 정규화할 것.
...
- **CJ 자동화 계층은 공식 API가 없는 외부 시스템이므로, 매번 실제 사이트를 두드리는 자동 테스트에 의존하지 않는다.**
  ...
  - **[예외]** 반복 예약 기능은 Windows 작업 스케줄러(사무실 PC)에서 **자정마다 1회**, 사용자별로 **순차 실행**(병렬 아님), **규칙당 최대 3회 시도**(1~3순위 회의실)의 제약 하에서만 CJ 사이트에 접근한다. 이는 브라우저 자동화의 필수 특성상 불가피한 예외이지만, 제약을 통해 실제 시스템 부하는 미미하게 유지한다(1일 회의실 개수×사용자 수 수준).
...
- **Access Token은 클라이언트 메모리(Zustand)에만 두고 `localStorage`에 저장하지 않는다.** Refresh Token은 httpOnly + Secure + SameSite 쿠키로만 저장한다 (PRD "인증/보안" 절 그대로).
  ...
  - 이렇게 하면 PRD가 이미 정한 httpOnly 보안 결정(XSS로부터 Refresh Token 보호)을 그대로 유지하면서 dev/prd 차이는 배포 구조로 흡수된다. Refresh Token을 다시 클라이언트가 읽을 수 있는 저장소(localStorage 등)로 내리는 것은, 이 서비스가 CJ WORLD 자격증명(암호화 보관)까지 다루는 점을 감안해 채택하지 않는다.
