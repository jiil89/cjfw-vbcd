...
- `docs/1-domain-definition-meeting-room-agent.md` — 도메인 정의서(최상위 source of truth). 온보딩 플로우, 유스케이스, 비즈니스 규칙, §9 CJ 시스템 API 명세
...
- `docs/laptop-server-setup.md` — **운영 배포 절차서(정본)**. CJ가 사내망 전용이라 클라우드 배포가 불가능해, 사내 전용 노트북 1대에서 프론트+백엔드+스케줄러를 돌리고 DB만 Supabase를 쓴다. 배포/재시작/업데이트 시 이 문서를 따를 것
