...
**CJ 예약 시스템(`CJ 예약 서버`)이 사내망에서만 접근 가능하다**(2026-08-19 확인).
...
| CJ 자동화(Playwright) | **이 노트북** | 사내망 필수 |
...
npm install
npx playwright install chromium      # CJ 자동화에 필요. 빠뜨리면 예약이 전부 실패한다
...
> **`CREDENTIAL_ENCRYPTION_KEY`는 절대 잃어버리면 안 된다.** 사용자들의 CJ 계정
...
(CJ가 7일 뒤까지만 예약을 받으므로, 예약 창이 열리는 순간 잡는 구조다.)
...
2. 회원가입 → 사번, CJ WORLD 비밀번호, 앱 로그인 비밀번호, 선호 회의실 입력
3. 로그인 → 챗봇 화면이 뜨면 성공
...
- [ ] 로그인 성공 (= CJ 자동 로그인까지 통과했다는 뜻)
- [ ] 챗봇으로 회의실 조회가 됨 (= CJ 연동 정상)
- [ ] 실제 예약 1건 생성 후 CJ 사이트에서 확인
