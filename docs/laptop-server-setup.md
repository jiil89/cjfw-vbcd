...
> **[20260821 추가]** 위 4개(`CJ_PORTAL_BASE_URL`~`CJ_FLOOR_AREA_CODES`)는 원래 코드에
...
>
> **[20260826 추가]** YTN 본사(2번째 사업장) 지원으로 `CJ_SITE_AREA_CODE`/`CJ_SITE_SUB_AREA_CODE`
> (단일 사업장 전제)가 `CJ_SITE_AREA_CODES`(사업장명:건물코드 콤마 구분)로 합쳐지고,
> `CJ_FLOOR_AREA_CODES`의 키에 사업장명 접두사가 붙었다(`.env.example` 참고). 기존
> `.env`를 복사해 쓰던 배포라면 이 두 변수를 새 형식으로 갱신해야 부팅된다.
