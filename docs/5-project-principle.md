...
이 문서는 `docs/4-prd.md`(PRD)와 `docs/1-domain-definition-meeting-room-agent.md`(도메인 정의서), `supabase/migrations/`(실제 DB 스키마)에서 이미 확정된 결정사항을 바탕으로, 실제 코드를 작성할 때 따라야 할 구조 원칙을 정리한 것이다. 여기 나오는 원칙은 전부 이 프로젝트의 실제 스택(React 19 + Zustand + TanStack Query, Node.js + Express + `pg`, Vercel, Supabase, Playwright)에 한정된다 — 범용 아키텍처론을 늘어놓지 않는다.

이 문서는 실제 구현이 진행되면서 계속 다듬어진다 — 실사용에서 발견된 버그의 재발 방지 규칙도 여기에 누적한다(진행 상태는 `docs/4-prd.md` 3번, 세부 이력은 `docs/9-plan.md` 참고).
...
- **두 종류의 "비밀번호"를 코드 레벨에서도 절대 섞지 않는다.** DB가 이미 `users.encrypted_password`(**사내 계정 PW**, 복호화 가능한 암호화)와 `users.app_password_hash`(이 서비스 로그인, 단방향 해시)를 분리해 놓았다. 애플리케이션 코드에서도:
  ...
  - **변경 경로도 따로 둔다** — 비밀번호 변경 API/폼을 하나로 합치지 않는다(`PATCH /me/cj-world-password` vs `PATCH /me/app-password`). 검증 방식 자체가 다르기 때문이다: 사내 계정 PW는 **외부 시스템에 실제 로그인해서** 맞는지 확인하고, 앱 비밀번호는 **저장된 해시와 비교**한다.
  - 사용자에게 보이는 명칭은 **사내 계정 시스템 ID / 사내 계정 PW**로 통일한다(DB 컬럼명은 그대로).
- **[20260816 추가] 외부 시스템 자격증명은 저장 전에 그 시스템으로 검증한다.** 사내 계정 PW를 검증 없이 저장하면 오타 하나로 이후 모든 조회·예약이 실패하는데, 사용자는 원인을 알 수도 되돌릴 수도 없다. 검증에 성공한 값만 저장하고, 옛 자격증명으로 맺어둔 세션 캐시는 즉시 버린다.
- **[20260816 추가] 복구 경로는 그 기능이 필요한 상태에서 도달 가능해야 한다.** 로그인은 세션 확보 실패 시 거부되므로, 사내 계정 PW 재등록을 **인증된 화면 안에만** 두면 정작 필요한 순간(비밀번호가 만료돼 로그인이 막힌 상태)에 쓸 수 없다. 그래서 로그인 전 경로(`POST /auth/cj-world-password`, 앱 로그인 비밀번호로 본인 확인)를 함께 둔다. **기능을 만들 때 "이게 필요한 상황에서 실제로 접근 가능한가"를 반드시 확인할 것.**
- **LLM은 판단만 하고, 실행은 항상 서버 코드가 한다.** LLM(gpt-5-nano)이 하는 일은 "사용자 발화 → 어떤 도구를 어떤 인자로 호출할지 결정"까지다. 실제 예약 시스템 호출, DB 쓰기, 검증 로직은 전부 결정론적인 서버 코드가 수행한다. LLM 응답을 그대로 실행 결과로 신뢰하지 않는다.
...
- **백엔드 내부 의존 방향을 한 방향으로 고정한다: LLM 오케스트레이션 계층 → 도구(서비스) 계층 → 자동화 계층.**
  - LLM 오케스트레이션 계층: 사용자 발화를 받아 도구 호출로 변환한다. 이 계층은 "도구 계층"의 함수만 호출할 수 있고, Playwright나 `pg`를 직접 알지 못한다.
  - 도구(서비스) 계층: `checkRoom → checkStraightRoom → checkDayCountLimit → SaveReserve` 같은 예약 도메인 규칙, 선호 회의실 우선순위, 분할 예약/보상 트랜잭션 로직이 여기 있다. 이 계층이 자동화 계층과 DB 리포지토리를 호출한다.
  - 자동화 계층: Playwright + `@sparticuz/chromium`으로 실제 `예약 서버` ASMX 엔드포인트를 호출하는 가장 하위 계층. 이 계층은 예약 비즈니스 규칙(2시간 제한 등)을 모르고, 순수하게 "로그인하고 이 API를 호출한다"만 안다.
  - 역방향 의존을 만들지 않는다 — 예를 들어 자동화 계층이 LLM을 호출하거나, 도구 계층이 프롬프트 문자열을 조립하는 일은 없어야 한다.
  - **[예외, 2026-08-14]** `routes/auth.routes.ts`는 로그인 시점에 세션을 미리 확보(예열)하려고 `cj-automation/session.ts`의 `getValidSession`을 도구 계층을 거치지 않고 직접 호출한다 — 이건 예약 도메인 로직이 아니라 "로그인 부수효과로 세션을 캐싱해두는" 횡단 관심사라 도구 계층에 억지로 끼워넣지 않기로 한 의도된 예외다(코드 주석에도 이유를 남김).
...
- **사내 계정 시스템 자격증명 복호화는 자동화 계층 안에서만 일어난다.** 다른 계층(도구 계층, API 라우트, 프론트엔드)은 복호화된 비밀번호를 절대 넘겨받지 않는다.
- **재로그인 전략을 계층 경계에 반영한다.** 도메인 정의서 9번의 "세션이 수 분 단위로 짧게 끊긴다"는 관찰에 따라, 자동화 계층은 매 요청 시작 시 세션 유효성을 스스로 확인하고 필요하면 재로그인한다 — 상위 계층(도구 계층)이 세션 상태를 알거나 관리하지 않는다.
...
- **사내 API의 원본 필드명(`area_code`, `room_code`, `seq` 등)은 자동화 계층 내부에서만 그대로 쓰고, 그 위 계층으로 넘어갈 때는 우리 도메인 이름(`roomId`, `cjSeq` 등 — `reservations.cj_seq` 컬럼명과 맞춤)으로 바꾼다.** 도메인 정의서 9번에 나온 대로 `area_code`가 건물/층 두 가지 의미로 혼용되는 등 원본 API 자체가 혼란스러우므로, 이 혼란이 상위 계층까지 전파되지 않게 막는 경계 역할을 자동화 계층이 담당한다.
...
- **자동화 계층은 공식 API가 없는 외부 시스템이므로, 매번 실제 사이트를 두드리는 자동 테스트에 의존하지 않는다.**
  - 자동화 계층의 각 함수(로그인, `checkRoom`, `SaveReserve` 등)는 도구 계층에서 인터페이스로 분리해, 도구 계층 테스트에서는 이 인터페이스를 페이크/스텁으로 대체한다.
  - 실제 사이트 대상 테스트는 자동화하지 않고, 스키마 변경 등 꼭 필요할 때만 수동으로 소수 계정으로 점검한다 (도메인 정의서 9번 리스크: 여러 브라우저의 동시 로그인이 예약 시스템에 이상 트래픽으로 감지될 가능성이 아직 검토되지 않았으므로, 자동화된 반복 실행으로 실제 시스템에 부하를 주지 않는다).
  - **[예외]** 반복 예약 기능은 Windows 작업 스케줄러(사무실 PC)에서 **자정마다 1회**, 사용자별로 **순차 실행**(병렬 아님), **규칙당 최대 3회 시도**(1~3순위 회의실)의 제약 하에서만 사이트에 접근한다. 이는 브라우저 자동화의 필수 특성상 불가피한 예외이지만, 제약을 통해 실제 시스템 부하는 미미하게 유지한다(1일 회의실 개수×사용자 수 수준).
...
- **분할 예약의 보상 트랜잭션(부분 실패 시 `delReserve`로 롤백)은 반드시 테스트한다.** 실사용자에게 가장 위험한 경로(예약이 일부만 생성된 채 남는 것)이므로, 자동화 계층을 스텁으로 중간 실패를 강제로 재현하는 테스트를 둔다.
...
- **[20260816 추가] UI/CSS는 "코드를 읽어서" 판단하지 말고 실제로 렌더해서 계산값을 확인한다.** 실사용 버그로 확인된 사례: `.chat-rail { display: none }`이 모바일 미디어쿼리 안에 분명히 있어서 "숨겨진다"고 판단했지만, 같은 특정도의 `.chat-rail { display: flex }`가 파일 뒤쪽에 있어 **한 번도 적용된 적이 없었다.** 소스를 읽는 것만으로는 절대 안 보이는 종류의 버그다.
  - 판정이 필요하면 해당 CSS를 그대로 링크한 최소 페이지를 띄우고 `getComputedStyle`/`getBoundingClientRect`로 **숫자를 확인**한다. "적용됐을 것"이라고 결론내지 않는다.
  - 로그인이 필요해 실제 화면을 띄울 수 없으면, 실제 CSS 파일을 링크한 임시 검증 페이지로 확인하고 **확인 후 삭제**한다(목업을 새로 그려서 검증하는 건 검증이 아니다 — 목업은 코드가 아니다).
...
- **`.env`는 Dev/Prd 모두 클라우드 Supabase를 가리킨다** (PRD 2번: "Dev/Prd가 항상 같은 Postgres를 쓰게 해서 환경 차이 문제를 줄인다"). `.env`에는 최소한 다음이 구분되어 존재해야 한다 — 절대 하나로 합치지 않는다:
  ...
  - `CREDENTIAL_ENCRYPTION_KEY` (사내 계정 비밀번호 암호화 키 — JWT 시크릿과 완전히 다른 키. 도메인 정의서 8번 Open Question대로, 이 키를 DB나 코드 저장소에 두지 않고 Vercel 환경변수로만 관리한다. KMS 도입 여부는 아직 미결정이지만, "암호문과 키를 같은 곳에 두지 않는다"는 원칙만은 지금부터 지킨다)
  - `OPENAI_API_KEY`, `OPENAI_MODEL` (기본값 `gpt-5-nano`, 도구 호출 정확도가 부족하면 `gpt-5-mini`로 교체 가능하도록 반드시 환경변수로 분리 — 코드에 모델명을 하드코딩하지 않는다)
- **비밀 관리 원칙: JWT 시크릿, 암호화 키, OpenAI 키를 서로 다른 환경변수로 완전히 분리한다.** 하나가 유출되어도 나머지 비밀의 안전에 영향을 주지 않아야 한다.
...
- **[20260816 추가] 오케스트레이션 세션도 같은 이유로 DB에 둔다.** `orchestration/sessionStore.ts`가 원래 모듈 전역 `Map`(순수 메모리)이었는데, Vercel 서버리스는 함수 인스턴스가 바뀌면 그 메모리가 통째로 사라진다 — 로컬은 프로세스가 계속 떠있어 이 문제가 절대 드러나지 않았다(배포 후에야 터지는 종류의 버그였다). `chat_sessions` 테이블(사용자당 1행, `state jsonb`로 세션 전체를 통째로 저장)로 옮겼다.
  ...
  - **의도된 예외**: `sessionStore.ts`가 `db/repositories/chatSessionRepository`를 직접 부른다. `orchestration → tools → db` 의존 방향을 우회하는 것처럼 보이지만, 오케스트레이션 자신의 상태를 어디에 영속화할지는 예약 비즈니스 로직이 아니라 순수 인프라 관심사라 tools/에 억지로 끼워넣지 않는다(로그인 시 세션을 예열하는 `auth.routes.ts`의 기존 예외와 같은 성격).
...
- **Access Token은 클라이언트 메모리(Zustand)에만 두고 `localStorage`에 저장하지 않는다.** Refresh Token은 httpOnly + Secure + SameSite 쿠키로만 저장한다 (PRD "인증/보안" 절 그대로).
  ...
  - 이렇게 하면 PRD가 이미 정한 httpOnly 보안 결정(XSS로부터 Refresh Token 보호)을 그대로 유지하면서 dev/prd 차이는 배포 구조로 흡수된다. Refresh Token을 다시 클라이언트가 읽을 수 있는 저장소(localStorage 등)로 내리는 것은, 이 서비스가 사내 계정 시스템 자격증명(암호화 보관)까지 다루는 점을 감안해 채택하지 않는다.
- **Vercel Hobby 플랜 제약을 코드/운영에 반영한다.** 함수 실행시간 최대 300초, 메모리 2GB (PRD 2번, 2026-07 기준). 자동화 함수(로그인+예약 API 호출)가 이 한도 안에서 끝나는지 특히 신경 쓰고, 트래픽이 늘어 한도에 근접하면 Pro 플랜 전환을 그때 검토한다 (지금 미리 대비 코드를 넣지 않는다).
- **콜드스타트/처리 지연에 대한 사용자 안내는 UI 원칙이자 운영 원칙이다.** 자동화가 헤드리스 브라우저를 매번 새로 띄우므로 응답까지 몇 초 걸릴 수 있다 — 챗봇 UI(`docs/design/chatbot-shell.html`, 시나리오 10번)에 "확인 중입니다" 같은 처리 중 표시를 반드시 넣는다.
...
- **[20260816 추가] 시스템 프롬프트는 "정적 내용 먼저, 턴마다 바뀌는 내용은 맨 끝"으로만 조립한다 (프롬프트 캐싱).** OpenAI 자동 프롬프트 캐싱은 입력의 앞부분이 이전 요청과 정확히 일치하는 구간까지만 재사용하고, 한 글자라도 다르면 그 뒤 전체가 캐시 미스가 된다. `orchestration/systemPrompt.ts`의 `pendingConfirmation`(제안 직후/확인 후로 매 턴 바뀜)을 프롬프트 중간에 끼워 넣었더니, 그 뒤에 오는 회의실 목록·응답 스타일 규칙까지(프롬프트 전체의 약 60%) 매 도구 호출마다 다시 계산되고 있었다 — 맨 끝으로 옮겨서 앞부분(정적 규칙+회의실 목록)이 확인 대기 상태와 무관하게 항상 동일한 문자열이 되게 했다(회귀 테스트: `systemPrompt.test.ts` "프롬프트 캐싱 적중률 보존"). **앞으로 시스템 프롬프트에 새 동적 값을 추가할 때 이 순서를 반드시 지킬 것** — 중간에 끼워 넣으면 캐시 적중률이 실사용 트래픽에서 조용히 나빠진다.
  - 회의실 목록은 5분 TTL 캐시(`orchestrator.ts`)라 상대적으로 안정적이지만, 이 순서 원칙 덕에 지금은 사용자/턴과 무관하게 완전히 정적이다.
  - `today`/날짜표(§2)는 하루에 한 번만 바뀌므로 앞쪽에 둬도 무해하다 — 일 단위 캐시 미스 정도는 감수할 만하다.
- **[20260816 추가] 턴당 OpenAI 토큰 사용량을 로그로 남긴다.** 이전에는 `usage`(prompt_tokens/completion_tokens) 필드를 받고도 버리고 있어서 실제 사용량을 알 방법이 없었다. `orchestrator.ts`가 도구 호출 루프(턴당 최대 6회) 안의 모든 OpenAI 호출 usage를 합산해 턴 끝에 한 줄로 로그를 남긴다.
...
Node.js + Express + `pg`, 2번의 레이어 원칙(LLM 오케스트레이션 → 도구 → 자동화)과 실제 도메인(인증, 예약, 자동화, Admin) 기준.
...
│  ├─ cj-automation/              # [레이어 3] 자동화 — Playwright, 최하위 계층
