...
`docs/`, `supabase/`에 정리된 확정 사항(도메인 정의서, PRD, ERD, 프로젝트 구조 원칙, 통합 스키마, 보안 검토 결과)을 근거로 DB/백엔드/프론트엔드 단위 Task로 분해한 실행계획이다. 각 Task는 작업 내용·선행 Task·체크박스형 완료 조건으로 구성된다.
...
- **완료 조건**:
  ...
  - [x] `supabase/8-schema.sql`에도 반영 (`docs/8-erd.md`도 함께 갱신)
...
- **완료 조건**:
  ...
  **[2026-08-15, 사용자 실사용 피드백 3건 — "아직도 대화가 잘 안되네" + "확인 버튼이 너무 많다"]** 사용자가 실제 대화록과 함께 3가지를 보고함. 각각 원인이 전혀 달랐다.
  1. **"차주 월요일"을 8월 24일로 잘못 계산** (실제로는 8월 17일). 원인: 시스템 프롬프트가 오늘 날짜 하나만 주고 **상대 날짜 계산 자체를 모델의 암산에 맡기고 있었다.** 이전 세션에서 요일 암산 실패를 겪고 `todayWeekdayKo()`로 요일만 미리 계산해 넣었는데, 그건 "오늘이 무슨 요일인지"만 해결했을 뿐 "차주 월요일이 며칠인지"는 여전히 모델이 계산해야 했다. → **`buildRelativeDateTable()` 신설**: 오늘부터 14일치를 `2026-08-17(월요일) = 4일 후, 다음주 월요일`처럼 라벨과 함께 전부 표로 만들어 프롬프트에 박아준다. 모델은 이제 계산하지 않고 표에서 찾기만 하면 된다. 회귀 테스트 추가(`systemPrompt.test.ts`).
  2. **내 예약 카드에 "회의실 미상"이 뜸.** 원인: 프롬프트/LLM 문제가 아니라 순수 백엔드 버그 — `myReservations.tool.ts`가 `roomName: null as string | null`로 **하드코딩**하고 회의실 이름을 아예 조회하지 않고 있었다(프론트의 `?? "회의실 미상"` 폴백이 항상 발동). 같은 파일이 쓰던 `findActiveReservationsByUserAndRange`에 room 조인이 없었던 게 원인인데, **이미 `findActiveReservationsWithRoomByUserAndRange`(rooms 조인 버전)가 존재**했으므로 그걸로 교체하고 `reservation.roomName`을 그대로 실어줌. 조인 없는 구버전 함수는 이 교체로 사용처가 0이 되어 삭제함.
  3. **"확인 버튼을 누르는 게 너무 많다"** — 사용자 원문: "해당 회의실 취소해줘 라고 보내면 다시 나에게 물어보는게 아니고 에이전트가 취소하면 되는거야. 어떤 리스트 있어? 하고 에이전트가 보여주고 내가 선택하면 바로 예약을하면되지, 한 번더 예약 버튼을 누르는건 비효율이다." 이건 버그가 아니라 **BE-7의 propose→confirm 2단계 안전장치와 정면으로 충돌하는 설계 요구**라, 임의로 안전장치를 약화시키지 않고 사용자에게 범위를 확인받고 진행함(선택: "목표가 명확할 때만 1클릭 생략"). 구현 방침은 **"확인 절차를 없애는 게 아니라, 서버가 객관적으로 검증할 수 있을 때만 생략한다"**:
     - `sessionStore`에 근거 2종 신설 — `offeredSlots`/`offeredSlotsTurn`(check_availability로 **실제로 사용자에게 보여준** 슬롯과 그 턴), `resolvedTargetReservationId`(find_reservation_candidates가 **후보 정확히 1건**으로 좁힌 예약 id).
     - `wasSlotOfferedBefore()`는 **이전 턴에 보여준 슬롯만** 인정한다 — 같은 턴에 조회하고 곧바로 예약하면 사용자는 목록을 본 적조차 없으므로 "선택"으로 볼 수 없다. 이 턴 조건이 이번 설계의 핵심 안전장치다.
     - `orchestrator.ts`의 confirm_* 4개 본문을 `runCreateReservation`/`runSplitReservation`/`runModifyReservation`/`runCancelReservation` 러너로 추출하고, propose_* 단계에서 위 근거가 성립하면 러너를 **그 자리에서 호출해 실행까지 끝낸다.** LLM에게 "같은 턴에 confirm을 부르라"고 지시하는 방식은 모델 신뢰에 의존하므로 택하지 않았다 — 서버가 직접 실행하므로 LLM이 무슨 판단을 하든 결과가 달라지지 않는다.
     - 프론트가 도구 이름으로 카드를 매핑하므로 `ToolExecutionResult.toolNameOverride`를 신설해 즉시 실행 시 결과를 `confirm_*` 이름으로 넘긴다 → **프론트 수정 없이** 기존 "완료 카드"가 그대로 그려진다.
     - **생략되지 않는 것(그대로 유지)**: 후보 2건 이상(ambiguous), 후보 0건, 분할 예약의 취소 범위 확인(`scope_required`), 그리고 근거 기록이 없는 모든 경우. `validatePendingConfirmation`의 "같은 턴 confirm 거부" 로직도 손대지 않았다 — LLM이 부르는 confirm_* 경로의 안전장치는 그대로다.
     - 도메인 정의서(`1-domain-definition-meeting-room-agent.md` 예약 변경/취소 절)에 이 규칙 변경을 먼저 반영하고 코드를 맞춤(CLAUDE.md 문서 우선 원칙).
  4. **"채팅은 친절하게 대답해라"** (같은 날 추가 피드백). 원인은 프롬프트 §5가 **분량 규칙만** 갖고 있었다는 것 — "1~3문장으로 끝낸다", "안내 문구를 덧붙이지 않는다"만 반복해서 지시하다 보니 모델이 짧게 답하려다 **무뚝뚝해지는 쪽으로 최적화**되고 있었다(말투에 대한 지시가 프롬프트 어디에도 없었음). §5를 **5-1 말투 / 5-2 분량**으로 분리하고 5-1을 신설:
     - 정중한 존댓말("~해요"체) 고정, 요청은 명령조가 아니라 부탁조로.
     - **거절·실패·빈 결과에서 딱 자르지 않기** — 이유 한 줄 + 가능한 대안 제시를 세트로 요구하고, X/O 예시를 나란히 넣음(형용사만 주는 것보다 예시가 훨씬 잘 먹힘). 같은 취지로 §0의 범위 밖 거절 문구("바로 ~지원하지 않는다고 안내하고 끝냅니다")와 §4의 회의실 미매칭 문구도 "정중히 알리고 대안 제시"로 고침.
     - 내부 용어(도구 이름/status/토큰/에러 코드/"서버","DB") 노출 금지를 명시.
     - **핵심 균형**: "친절함은 말투로 표현하고 분량으로 표현하지 말 것"을 명시해 5-2의 1~3문장 제한을 그대로 유지시킴 — 사과/배려를 이유로 문장이 길어지면 카드와 내용이 중복되는 원래 문제가 되살아나기 때문. 테스트도 이 균형(친절 규칙 + 분량 제한이 **둘 다** 존재)을 함께 검증한다.
     - 부가 정리: `SystemPromptParams.userDisplayName`은 선언만 되고 **한 번도 전달된 적 없는 죽은 파라미터**였고(호출부는 `today`/`rooms`/`pendingConfirmation`만 넘김), 호칭에 쓸 실명 필드가 스키마에 아예 없어서(`users`에는 `email_alias`뿐) 제거함. 이름으로 부르는 친절함이 필요해지면 실명 컬럼 추가부터 논의해야 한다.
  5. **"모바일에서 아래쪽이 공간을 많이 차지한다"** (사용자 요청: 작업 전 디자인 먼저 보여주고 컨펌받을 것). 먼저 확인한 사실 — **모바일에서 사이드바(`.chat-rail`)는 이미 `display:none`**이므로(`ChatPage.css:162`) 하단을 차지하는 건 **컴포저 하나뿐**이었다. 실측 결과 컴포저가 항상 **약 138px**(칩 32+여백 12 / 입력 46 / "Enter로 보내기" 20 / 상하 패딩 28)을 차지해, 375×667에서 키보드가 올라오면 대화가 한두 개밖에 안 보였다.
     - `docs/design/mobile-composer-proposal.html`에 현재/A안/B안을 **실제 디자인 토큰으로** 나란히 그린 목업을 만들어 컨펌을 받음(같은 폰 프레임에서 보이는 메시지 수가 3 → 5 → 7개로 늘어나는 걸 그대로 보여줌). **사용자가 B안(최대 축소) 선택.**
     - 구현(모바일 ≤860px에서만, 데스크톱은 무변경): 빠른명령 칩 줄을 **"+" 토글로 접고**(`isQuickOpen` 상태 + `.chat-quick-chips.is-open`), **"Enter로 보내기" 안내 삭제**(모바일 키보드에는 Enter 전송 개념이 없어 무의미), 입력/전송 46→44px(터치 목표 최소 권장치라 그 아래로는 안 줄임), 좌우 패딩 24→16px, **`env(safe-area-inset-bottom)` 추가**(아이폰 홈 인디케이터 겹침 방지 — 기존에 빠져 있었음). 펼쳤을 때는 가로 스크롤 대신 `flex-wrap`으로 칩이 한눈에 다 보이게 함.
     - 검증: 로그인이 필요해 실제 챗 화면을 띄울 수 없으므로, **작성한 CSS 원본을 그대로 링크한** 임시 검증 페이지로 375×667에서 실측 — 접힘 **67px**(약속한 66px), 펼침 107px, 입력폭 239px(플레이스홀더가 잘리지 않아 문구는 그대로 유지), 칩 3개가 한 줄에 모두 들어감. 데스크톱 1280px에서는 140px/46px/토글 숨김/안내문구 유지로 **완전 무변경** 확인. 검증 페이지는 확인 후 삭제.
     - 이 과정에서 버그 하나를 잡음: 펼침 표시를 `.chat-quick-toggle`에 `transform: rotate(45deg)`로 걸었더니 **테두리까지 돌아가 버튼이 마름모로 보였다**(스크린샷으로 발견). 회전을 내부 `svg`에만 걸도록 수정.
     - **[2026-08-16 정정 — 진짜 원인은 따로 있었다]** 사용자가 iPhone 12 Pro(390px) 스크린샷을 보내와서 확인하니, 컴포저 축소는 정상 적용됐는데 그 **아래에 `오늘 예약`/`선호 회의실`/안내 배너(= 사용자가 말한 "아래쪽 세팅값들")가 그대로 쌓여 있었다.** 즉 원래 요청은 컴포저가 아니라 **rail**에 대한 것이었고, 나는 `.chat-rail { display:none }`이 모바일 미디어쿼리 안에 있는 걸 읽고 "이미 숨겨져 있다"고 잘못 결론냈다. 실제로는 **CSS 순서 버그**로 그 규칙이 한 번도 작동한 적이 없었다 — 숨김 규칙은 파일 앞쪽(`.chat-body` 미디어쿼리, 162줄)에 있는데 `.chat-rail { display: flex }` 기본 규칙이 **훨씬 뒤(928줄)** 에 있어 특정도가 같은 채로 항상 덮어쓰고 있었다. 그래서 모바일에서 rail이 채팅 아래로 쌓여 화면 절반 가까이를 먹고 있었다.
       - 수정: 숨김 규칙을 **기본 규칙 바로 뒤**로 옮기고, 순서 의존성을 주석으로 못 박음. 앞쪽 미디어쿼리에는 "여기 두면 덮어써진다"는 경고 주석만 남김.
       - 검증(390×844, 실제 CSS): rail `display:none`/높이 0, 컴포저 67px이 화면 맨 아래(bottom=844)에 정확히 붙음. 데스크톱 1280px에서는 rail 300px 우측 정상 + `+` 토글 숨김 유지.
       - 교훈: **미디어쿼리 안의 규칙을 봤다고 그게 실제로 적용된다고 단정하면 안 된다.** 같은 특정도의 기본 규칙이 파일 뒤쪽에 있으면 조용히 덮어써진다. 코드를 읽고 판단하지 말고 실제로 렌더해서 계산값(`getComputedStyle`)을 확인할 것.
  ...
  **[2026-08-16, 프롬프트 캐싱 적용 + 턴당 토큰 사용량 로깅]** 사용자 질문("현재 llm api input/output 토큰이 대략 얼마씩 사용되나?")에 답하다 두 가지가 드러남: (1) OpenAI 응답의 `usage` 필드를 받고도 버리고 있어서 실제 사용량을 로깅한 적이 한 번도 없었다. (2) 실제 시스템 프롬프트를 만들어 재보니(현재 DB 기준 회의실 26개) 9,632자 — 도구 스키마(9,639자 JSON)까지 합쳐 매 도구 호출마다 그대로 재전송되고 있었는데, 뒤이은 사용자 요청("프롬프트 캐싱 방법을 적용하면 좋을텐데")으로 캐싱 여지를 조사함.
  - **발견**: OpenAI 자동 프롬프트 캐싱은 "앞부분이 이전 요청과 정확히 일치하는 구간까지만" 재사용하고 한 글자라도 다르면 그 뒤 전체가 캐시 미스가 되는데, `systemPrompt.ts`의 `pendingConfirmation`(제안 직후/확인 후로 매 턴 바뀜)이 프롬프트 **중간**(§3-5b 다음, 전체의 약 40% 지점)에 끼워져 있었다. 실측: pendingConfirmation 유무에 따른 두 프롬프트의 공통 접두사 길이를 직접 계산해보니, 수정 전 구조라면 §3-6부터 끝(회의실 목록·응답 스타일 규칙 포함, 전체의 약 60%)까지 매번 무효화되는 상황이었다.
  - **수정**: `pendingSection`을 반환 문자열의 **맨 끝**으로 이동. 실측 재검증 결과 pendingConfirmation 유무와 무관하게 **공통 접두사가 9,633자(=전체 정적 내용) 100%**가 됨 — 이제 확인 대기 상태와 무관하게 앞부분 전체가 항상 동일한 문자열이다. `today`/날짜표(§2, 하루 1회만 바뀜)는 앞쪽에 남겨둠(무해한 일 단위 캐시 미스). 회귀 테스트 추가: "pendingConfirmation 유무와 무관하게 프롬프트 앞부분이 완전히 동일하다".
  - **토큰 로깅**: `callOpenAi`가 `usage`를 함께 반환하도록 바꾸고, `handleUserMessage`가 턴 안의 도구 호출 루프(최대 6회) 전체 호출의 usage를 합산해 턴 끝에 한 줄로 로그(`prompt=... completion=... total=...`)를 남기도록 추가 — 다음 실사용 대화부터 정확한 실측값이 서버 로그에 남는다.
  - `5-project-principle.md` §5에 "시스템 프롬프트는 정적 내용 먼저, 턴마다 바뀌는 내용은 맨 끝" 원칙과 토큰 로깅 원칙을 추가 — 나중에 새 동적 값을 프롬프트에 추가할 때 이 순서를 지키도록 못박음.
  - 검증: 백엔드 `tsc` + `vitest run`(113/113, 신규 1건) 통과. 실제 DB의 회의실 데이터로 시스템 프롬프트를 직접 빌드해 공통 접두사 비율을 실측(100%) — 목업이 아니라 실제 코드 경로로 확인.
